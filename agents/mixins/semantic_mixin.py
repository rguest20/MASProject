...
class SemanticMixin:
    ...
    def _ensure_semantic(self):
        ...
        if "links" not in sem or not isinstance(sem["links"], defaultdict):
            print(f"[FIX][A{self.id}] Repairing semantic['links']")
            sem["links"] = defaultdict(lambda: defaultdict(float))
    ...
    def _link(self, a, b, w):
        ...
        # Because links is defaultdict(lambda: defaultdict(float)),
        # these both create rows if missing.
        row_a = links[a]
        row_b = links[b]

        # Slight asymmetry if you want it (or make both += w)
        row_a[b] = row_a.get(b, 0.0) + w
        row_b[a] = row_b.get(a, 0.0) + w
