...
def scale(u, s): return [a*s for a in u]
def lerp(u, v, a): return add(u, scale(sub(v, u), a))
...
def extract_keywords(text, k=12):
    if not text: return []
...
def sniff_dictionary_keys(raw_json, max_keys=400):
    try:
